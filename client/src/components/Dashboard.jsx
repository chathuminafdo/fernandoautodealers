...
function KpiCard({
  ...
  return (
    <div
      className="kpi-card"
      style={{ "--ka": accent, animationDelay: `${delay}ms` }}
    >
      ...
      {tag && (
        <div className="kpi-tag" style={{ color: tagColor }}>
          {tag}
        </div>
      )}
...
function TargetSection({ target, thisMonth, thisMonthCount }) {
  ...
  const profitPct =
    target.profit_target > 0
      ? Math.min(100, (thisMonth / target.profit_target) * 100)
      : 0;
  const unitsPct =
    target.units_target > 0
      ? Math.min(100, (thisMonthCount / target.units_target) * 100)
      : 0;
      ...
      <div className="card-header">
        <h3>
          <i
            ...
            style={{ marginRight: 8, color: "var(--r)" }}
          />
          MONTHLY TARGET
        </h3>
        <span
            ...
            ({
              ...
              <div key={label} className="target-item">
                <div className="target-labels">
                  ...
                  >
                    <i className={`fa ${icon}`} style={{ color: ic }} />
                    {label}
                  </span>
                  <span
                    style={{
                ...
                >
                  <span style={{ fontSize: ".62rem", color: oc }}>
                    {status}
                  </span>
                  <span
                    style={{
...
function MonthlyChart({
  ...
  const maxP = monthly.reduce(
    (m, r) => Math.max(m, Math.abs(parseFloat(r.profit) || 0)),
    1,
  );
  const tmPct =
    lastMonth > 0
    ...
    <div className="card" style={{ marginBottom: 16 }}>
      <div className="card-header">
        ...
        <span style={{ fontSize: ".68rem", color: "var(--t3)" }}>
          Last 12 months · Hover for detail
        </span>
      </div>
      <div className="month-stats">
        {[
          {
            lbl: "This Month",
            val: `LKR ${fmt(thisMonth)}`,
            sub: tmPct,
            sc: thisMonth >= lastMonth ? "var(--g)" : "var(--rl)",
          },
          {
            lbl: "Last Month",
            val: `LKR ${fmt(lastMonth)}`,
            sub: "Previous period",
            sc: "var(--t3)",
          },
          {
            lbl: "Monthly Avg",
            val: `LKR ${fmt(avgMonthly)}`,
          ...
          <div key={lbl} className="mstat">
            ...
            <div className="mstat-val" style={vc ? { color: vc } : {}}>
              {val}
            </div>
            <div className="mstat-sub" style={{ color: sc }}>
              {sub}
            </div>
          </div>
        ))}
              ...
              <div key={m.month_key} className="chart-col">
                <div className="chart-bar-wrap">
                  ...
                  >
                    <div
                      className="chart-val"
                      style={{ color: isPos ? "var(--g)" : "var(--rl)" }}
                    >
                      {sv}
                    </div>
                  </div>
                ...
                <div className="chart-lbl">
                  {parts[0]}
                  <br />
                  <span style={{ fontSize: ".53rem" }}>{parts[1] || ""}</span>
                </div>
              </div>
            );
          })}
...
function AgingHudCard({ count, label, tag, color, glow, pct, delay, pulse }) {
  ...
  return (
    <div
      className={`ahc ${pulse && count > 0 ? "ahc-crit" : ""}`}
    ...
    >
      ...
      <div className="ahc-bottom">
        ...
        <div className="ahc-bar">
          <div
            className="ahc-bar-fill"
            style={{ width: `${pct}%`, animationDelay: `${delay + 320}ms` }}
          />
        </div>
      </div>
    </div>
        ...
        <div className="aging-top-section">
          ...
          {aging.top.map((v, i) => {
            ...
            return (
              <div
                key={v.id}
                className={`aging-top-row ${isCrit ? "atr-crit" : ""}`}
    ...
    <div className="card">
      ...
      <div>
        {brands.slice(0, 8).map((b, i) => {
          const margin =
            b.total_sales > 0
              ? ((b.total_profit / b.total_sales) * 100).toFixed(1)
              : "0";
            ...
            >
              <div>
                ...
                <div className="brand-meta">
                  {b.sold} sold · {b.inhand} in hand · {b.onway} on way
                </div>
              </div>
...
export default function Dashboard({ showToast }) {
      ...
      .catch((err) => {
        if (active) {
          ...
          setData({
            ...
            loss_count: 0,
            target: null,
            aging: { fresh: 0, d30: 0, d60: 0, d90: 0, top: [] },
            monthly: [],
            recent: [],
            brands: [],
            msgs: 0,
  ...
  const winRate =
    data.sold > 0 ? ((data.profit_count / data.sold) * 100).toFixed(0) : "0";
  const chg =
    data.last_month > 0
    ...
    <>
      ...
      <div className="kpi-grid">
        <KpiCard
          ...
          accent="#22c55e"
          delay={0}
          tag={`From ${data.sold} vehicle sales`}
          tagColor="var(--t3)"
        />
        <KpiCard
          num={data.this_month}
          ...
          tag={chg}
          tagColor={
            data.this_month >= data.last_month ? "var(--g)" : "var(--rl)"
          }
        />
        <KpiCard
          num={data.total_sales}
          prefix="LKR "
          label="Total Revenue"
          icon="fa-chart-line"
          accent="rgba(255,255,255,.4)"
          delay={140}
          tag="Gross turnover"
          tagColor="var(--t3)"
        />
        <KpiCard
          ...
          accent="var(--g)"
          delay={210}
          tag={`${data.profit_count} profit · ${data.loss_count} loss`}
          tagColor="var(--t3)"
        />
        <KpiCard
          num={data.inhand}
          ...
          accent="var(--rl)"
          delay={280}
          tag={`Cost LKR ${fmt(data.inhand_cost)}`}
          tagColor="var(--t3)"
        />
        <KpiCard
          num={data.onway}
        ...
        <div className="card">
          <div className="card-header">
            ...
            <button
              ...
              onClick={() =>
                navigate("/vehicles", { state: { tab: "profit" } })
              }
            >
                ...
                {data.recent && data.recent.length > 0 ? (
                  data.recent.map((r) => (
                    <tr key={r.no}>
                      ...
                      <td>
                        <span
                          className={`badge ${r.type === "LOCAL" ? "b-local" : "b-import"}`}
                        >
                          {r.type}
                        </span>
                      </td>
                      ...
                      <td>
                        {r.income == null ? (
                          ...
                          <span
                            className={`badge ${parseFloat(r.income) < 0 ? "b-loss" : "b-profit"}`}
                          >
                            {parseFloat(r.income) < 0 ? "− " : "+ "}
                            {fmt(Math.abs(r.income))}
                          </span>
                        )}
                      </td>
